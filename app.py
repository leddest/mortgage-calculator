...
dropdown = dbc.DropdownMenu(
    children=[
        dbc.DropdownMenuItem("פורום משכנתאות", href='https://www.facebook.com/groups/mashkanta1/', style={"text-align": "right"}),
        dbc.DropdownMenuItem(
            "נתונים וריביות", href='https://www.boi.org.il/he/BankingSupervision/Data/Pages/Default.aspx', style={"text-align": "right"}),
        dbc.DropdownMenuItem(
            "מערכת נתוני אשראי", href='https://www.creditdata.org.il/', style={"text-align": "right"}),
            ...
            html.A(
                dbc.Row(
                    [
                        ...
                        dbc.Col(dbc.NavbarBrand("Mortgage Dashboard", className="ml-2")),
                    ],
                    align="center",
                    no_gutters=True,
                ),
...
def gen_maslul(index):
    return html.Div([
        dbc.Row(dbc.Input(id=f"inputTitle{index}", placeholder="שם המסלול", type="text", maxLength="15", style={
    ...
    dbc.Row(html.P(id=f'pmt{index}', style={'margin': 'auto', 'width': '88%', 'textAlign':"center"})),
    dbc.Row(html.P(id=f'total_pmt{index}', style={'margin': 'auto', 'width': '88%', 'textAlign':"center"})),  
    dbc.Row(dcc.Graph(id=f'output{index}', style={'margin': 'auto', 'width': '100%'}))
])
...
def display_value(schedule1, cpi1, inf1, amount1, i1, per1, schedule2,  cpi2, inf2, amount2, i2, per2,
    ...
    fig_payments = go.Figure(data=[
        go.Bar(name='תשלום ריבית', x=df_total["חודש"], y=df_total['תשלום ריבית'].round(
            decimals=2), marker=Marker(color='rgb(90, 202, 138)')),
        go.Bar(name='תשלום קרן', x=df_total["חודש"], y=df_total['תשלום קרן'].round(
...
def loan_callback(i):
    ...
    def display_value(schedule, cpi, madad, amount, i, period):
        maslul_df, total_ipmt_nominal = generate_pd_per_maslul(schedule, cpi, madad, amount, i, period)
        ...
        return fig, f"תשלום חודשי ראשוני:  {round(maslul_df[pmt_heb][0], 2):,}" , f" החזר בסוף תקופה: {round(maslul_df[pmt_heb].sum(), 1):,}"
